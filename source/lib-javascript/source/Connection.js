...
Connection.prototype.request = function (params) {
  ...
  var request = utility.request({
    ...
    path : '/' + this.username + params.path,
    headers : headers,
    payload : payload,
...
/**
 * @typedef AccessInfo
 * @see http://pryv.github.io/reference.html#data-structure-access
 */
...
Connection.login = function (params, callback) {
  ...
  var pack = {
    ...
    ssl: true,
    host:  domain,
    path: '/' + params.username + '/auth/login/',
...
function getHostname(connection) {
  return connection.settings.hostname || connection.settings.domain;
}
